...
# ax.set_title('L1 Regularization with IRLS\n $u(t,x)$ - ' + str(ind) + ' epochs', fontsize=18)
ax.set_title('$L^1$ Regularization with IRLS\n $\\rho (t,x)$', fontsize=18)
...
ax.plot(x, Exact_rho[0, :], 'b-', linewidth=2, label='Exact')
ax.plot(x, Rho_pred[0, :], 'r--', linewidth=2, label='Prediction')
ax.set_xlabel('$x$')
...
ax = plt.subplot(gs1[0, 0])
ax.plot(x, Exact_E[0, :], 'b-', linewidth=2, label='Exact')
ax.plot(x, E_pred[0, :], 'r--', linewidth=2, label='Prediction')
ax.set_xlabel('$x$')
ax.set_ylabel('$E(t,x)$')
ax.set_title('$t = 0$', fontsize=18)
#ax.set_xlim([-0.1, np.pi + 0.1])
