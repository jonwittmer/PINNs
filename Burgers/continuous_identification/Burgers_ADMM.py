...
class PINNWrapper:
    ...
    def plot_results(self):
        ...
        self.U_pred = griddata(self.X_star, self.u_pred.flatten(), (self.X, self.T), method='cubic')
...
class PhysicsInformedNN:
    ...
    def __init__(self, caller):
        ...
        # break up data
        self.x_data = caller.X_u_train[:, 0:1]
        self.t_data = caller.X_u_train[:, 1:2]
        self.u = caller.u_train

        # Save dimensions
        self.N_u = caller.params.N_u
        self.N_f = caller.params.N_f

        # list of number of nodes in each layer - first is input layer
        self.layers = caller.layers
        ...
        self.x_phys = np.random.uniform(self.lb[0], self.ub[0], [self.caller.params.N_f,1] )
        self.t_phys = np.random.uniform(self.lb[1], self.ub[1], [self.caller.params.N_f,1])
    ...
    def initialize_ADMM(self):
        ...
        self.rho = tf.constant(self.caller.params.rho)
