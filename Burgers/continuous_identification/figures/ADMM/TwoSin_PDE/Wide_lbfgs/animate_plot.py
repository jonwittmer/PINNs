...
original_data = scipy.io.loadmat('../../../../../Data/TwoSin_burgers_shock.mat')
...
gs0.update(top=1 - 0.1, bottom=1 - 1.0 / 2 + 0.06, left=0.15, right=0.85, wspace=0)
...
ax_top.plot(t[50] * np.ones((2, 1)), line, 'w-', linewidth=1)
ax_top.plot(t[-2] * np.ones((2, 1)), line, 'w-', linewidth=1)

ax_top.set_xlabel('$t$')
ax_top.set_ylabel('$x$')
ax_top.legend(loc='upper center', bbox_to_anchor=(1.0, -0.125), ncol=5, frameon=False)
ax_top.set_title('$$L^1$ Regularization with ADMM \n $$u(t,x)$ - ' + str(ind) + ' epochs', fontsize=18)
...
second_line, = ax.plot(x, U_pred[50, :], 'r--', linewidth=2, label='Prediction')
ax.set_xlabel('$x$')
ax.set_ylabel('$u(t,x)$')
#ax.axis('square')
ax.set_xlim([-1.1, 1.1])
ax.set_ylim([-1.1, 1.1])
ax.set_title(f'$t = {np.round(t[50],2)}$', fontsize=18)
ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.35), ncol=5, frameon=False)

ax = plt.subplot(gs1[0, 2])
ax.plot(x, Exact[-2, :], 'b-', linewidth=2, label='Exact')
third_line, = ax.plot(x, U_pred[-2, :], 'r--', linewidth=2, label='Prediction')
ax.set_xlabel('$x$')
ax.set_ylabel('$u(t,x)$')
#ax.axis('square')
ax.set_xlim([-1.1, 1.1])
...
def animate(i):
    ...
    second_line.set_data(x, U_pred[50, :])
    third_line.set_data(x, U_pred[-2, :])
    ax_top.set_title('$$L^1$ Regularization with ADMM \n $$u(t,x)$ - ' + str(int(all_epochs[i])) + ' epochs', fontsize=18)
